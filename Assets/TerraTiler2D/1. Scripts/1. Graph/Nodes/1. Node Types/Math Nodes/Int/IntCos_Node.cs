...
{
    public class IntCos_Node : Math_Node
    {
        private PortWithField<int> portA;

        private Port<float> outputPort;

        public IntCos_Node(string nodeName, Vector2 position, string guid = null) : base(nodeName, position, guid)
        {
            nodeType = Glob.NodeTypes.IntCos;
        ...
        {
            return (float)Mathf.Cos((int)portA.GetPortVariable());
        }
    }
}
